    ...
    <DashboardLayout>
      ...
      <div  class="dashbpoard-content px-0 px-md-5 py-4">
        ...
        <div class="total-department">
          <h5 class="mb-0">Total Documents = {{doc_obj.length}}</h5>
        </div>
        </div>
        <!-- *******team-content******* -->
        <div class="row">
                              ...
                              <form @submit.prevent="apply_filter_g">
                                <div class="mb-3 modal-input">
                                  ...
                                  <select class="form-select shadow-none" v-model="role_name_filter" aria-label="Default select example">
                                    ...
                                    <option v-for="data,index in doc_obj" :key="index">{{data.role_name}}</option>
                          ...
                          <form @submit.prevent="add_document">
                            <div class="mb-3 modal-input">
                              ...
                              <input
                                ...
                                class="form-control"
                                id="recipient-name"
                                v-model="doc_title.title"
                                placeholder="Document title is required"
                              />
                               <span class="text-danger" v-if="v$.doc_title.title.$error">{{
                                v$.doc_title.title.$errors[0].$message
                              }}</span>
                            </div>
                    ...
                    <div class="table-responsive department-table">
                      <table class="table align-middle">
                        ...
                        <tbody> 
                          ...
                          <tr class="text-center department-action" v-for="(doc,index) in doc_obj" :key="index">
                            <td>{{doc.id}}</td>
                            <td>{{doc.doc_title}}</td>
                            <td class="text-start">
                                  ...
                                  >
                                    ...
                                    >
                                      <div class="modal-content">
                                        ...
                                        <div class="modal-body modal-form">
                                          <form @submit.prevent="update_doc(update_r.id)">
                                            <div class="mb-3 modal-input">
                                              <label
                                                for="recipient-name"
...
export default {
   ...
   setup() {
      ...
      validations(){
         return {
          doc_title:{
             title: {
          required: helpers.withMessage("Document name is required", required),
        },
  ...
  mounted(){
    this.processChange = this.debounce(() => this.get_doc_type());
  ...
  methods:{
    ...
    update_select(doc){
           console.log(doc);
           this.update_r.id = doc.id;
           this.update_r.role = doc.doc_title;
    },
    //update data 
   async update_doc(id){
      ...
      if (!result) {
        return;
             ...
             this.loadingupdate = true;
              let data={
                doc_title:this.update_r.role,

              }
               console.log(data);
                let response = await ApiClass.updateNodeRequest("document-type-update?id="+id , true, data);
    ...
    //reset validation
    reset_validaton(){
           this.v$.doc_title.$reset();
      this.doc_title.title  = "";
    },
     
    //for filter get all role
      async get_doc_type() {
      let response = await ApiClass.getNodeRequest("document-type-get", true);
      ...
      if (response?.data) {
        ...
        if (response?.data?.status_code == "1") {
          console.log(response.data);
            ...
            this.doc_obj = response?.data?.data ?? [];
    ...
    async add_document(){
       ...
       const result = await this.v$.doc_title.$validate();
      if (!result) {
        return;
      }
      ...
      let form_data = {
        ...
        doc_title:this.doc_title.title,
      };
       console.log(form_data)
      let response = await ApiClass.postNodeRequest("document-type-create", true, form_data);
      console.log(response.data);
      this.loading = false;
    ...
    //delete api 
     delete_doc_data(id) {
        // alert(id)
          ...
          async delete_doc(id){
            // alert(id)
          let response = await ApiClass.deleteNodeRequest("document-type-delete?id="+id , true);
